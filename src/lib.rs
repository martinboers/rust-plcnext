...
// TODO: Logging using env_logger. Then redirect stdout to a file in the /opt/plcnext/logs directory
...
static mut SYSTEM: System = System { callback: None };
...
impl System {
    ...
    extern fn handle_event(operation: plcnext_sys::PlcOperation) {
        ...
        unsafe { if let Some(callback) = SYSTEM.callback.as_mut() {
    ...
    pub fn read_from_axio_to_gds(&self, timeout: u32) -> Result<(), String> {
    ...
    pub fn read_input_data(&self, fb_io_system_name: &str, port_name: &str, value: &[u8]) -> Result<(), String> {
    ...
    // TODO: Extract the fb_io_system_name from the port name.
    // TODO: Remove fb_io_system_name parameter.
    pub fn write_output_data(&self, fb_io_system_name: &str, port_name: &str, value: &[u8]) -> Result<(), String> {
        ...
        if !unsafe { plcnext_sys::ArpPlcGds_BeginWrite(gds_buffer, &mut data_buffer_page) } {
            ...
            // Try to end the write operation
    ...
    /// * 'timeout' - Timeout in milliseconds to wait for the event to be processed.
    ///               If zero, the calling thread will block forever.
    pub fn write_from_gds_to_axio(&self, timeout: u32) -> Result<(), String> {
...
/// Locks the buffer for reading and returns address of the current read buffer data.
/// TGdsBuffer* pGdsBuffer Handle to TGdsBuffer
/// char** pOutCurrentReadData The reference to the current read buffer data segment
/// is returned by this pointer. NULL, if an error occurs.
/// Returns true if data is valid and false if data is invalid.
/// If an internal error occurs during the operation this functions returns false and
/// sets *pOutCurrentReadData to NULL. In this case get the last error message with 
///  get_last_error().
// pub fn begin_read()

// ArpPlcGds_BeginRead(
//     pGdsBuffer: *mut TGdsBuffer,
//     pOutCurrentReadData: *mut *mut ::std::os::raw::c_char
//     ) -> bool {
...
fn get_gds_port(fb_io_system_name: &str, port_name: &str)
    -> Result<(*mut plcnext_sys::TGdsBuffer, usize), String> {
...
fn begin_gds(operation: GdsOperation) {
    // TODO: match operation - read or write
        // Begin write operation by getting a pointer to the GDS data buffer page
        // After this call, the GDS buffer will be locked
        let mut data_buffer_page: *mut c_char = std::ptr::null_mut();
        if !unsafe { plcnext_sys::ArpPlcGds_BeginWrite(gds_buffer, &mut data_buffer_page) } {
        ...
        // TO DO: Make this mutable or immutable, depending on whether its a read or write operation.
        // (May need to take this out of this function)
        let port_data = unsafe { &mut*( slice::from_raw_parts_mut(data_address, value.len()) as *mut [u8] ) };

}

fn end_gds(operation: GdsOperation) {
    // TODO: match operation - read or write
        // Unlock and release the GDS data buffer page
        if !unsafe { plcnext_sys::ArpPlcGds_EndWrite(gds_buffer) } {
            // Log::Error("ArpPlcGds_EndWrite failed");
...
fn release_gds_buffer(gds_buffer: *mut plcnext_sys::TGdsBuffer) -> Result<(), String> {
